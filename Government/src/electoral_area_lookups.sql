...
(SELECT
 ...
 c.clab_id,
 c.community_name,
 p.bc_electoral_district_id AS prov_riding_id,
 p.electoral_district_name AS prov_riding_name,
...
(SELECT
 ...
 c.clab_id,
 c.community_name,
 p.fednum AS federal_riding_id,
 p.enname AS federal_riding_name,
 SUM(st_area(c.geom)) / 10000 AS community_ha,
 SUM(st_area(st_intersection(c.geom, p.geom))) / 10000 AS overlap_ha,
 SUM(st_area(st_intersection(c.geom, p.geom))) / SUM(st_area(c.geom)) AS overlap_pct
FROM mit.communities_ply c
